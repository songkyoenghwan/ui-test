import type { LocalizedText, SupportedLang, SupportedLanguages } from './common/locale';
...
export type TourDestinationSortOrder = 'latest' | 'nameAsc';

export interface TourDestinationEntryResponse {
	id: number;
	name: string;
	thumbnailUrl: string;
...
export interface TourDestinationOnboardingItemResponse {
	id: number;
	title: string;
	description: string;
...
export interface TourDestinationOnboardingResponse {
	destinationId: number;
	hasBasicSurvey: boolean;
	items: TourDestinationOnboardingItemResponse[];
}

export type BasicSurveyQuestionType = 'SINGLE' | 'MULTI';

export interface TourDestinationBasicSurveyOptionResponse {
	id: number;
	label: string;
}

export interface TourDestinationBasicSurveyQuestionResponse {
	id: number;
	questionType: BasicSurveyQuestionType;
	aiRecommendationEnabled: boolean;
	title: string;
	options: TourDestinationBasicSurveyOptionResponse[];
}

export interface TourDestinationBasicSurveyResponse {
	destinationId: number;
	questionCount: number;
	questions: TourDestinationBasicSurveyQuestionResponse[];
}

export interface BasicSurveySkipRequest {
	submissionId: string;
	visitorId: string;
	languageCode: SupportedLang;
}

export interface BasicSurveyAnswerRequest {
	questionId: number;
	optionIds: number[];
}

export interface BasicSurveySubmitRequest extends BasicSurveySkipRequest {
	answers: BasicSurveyAnswerRequest[];
}

export type BasicSurveySubmissionStatus = 'SUBMITTED' | 'SKIPPED';

export interface BasicSurveySubmissionResponse {
	submissionId: string;
	visitorId: string;
	destinationId: number;
	languageCode: SupportedLang;
...
export interface TourDestinationListQuery extends PaginationQuery {
	/** 정렬 기준 (최신순, 가나다순) */
	sortOrder?: TourDestinationSortOrder;
...
/** v1 맵 부트스트랩용 대상지 스칼라 메타 */
export interface TourDestinationDetailResponse {
	id: number;
	name: string;
	latitude: number;
	longitude: number;
