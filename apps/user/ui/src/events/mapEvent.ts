...
import { boundsFromPoints, expandBoundsForOverlayPadding, measureWalkingRouteOverlayPadding } from '@/utils/map-bounds';
...
export default class MapController {
	...
	public facilityPins: Map<number, any>;
	...
	private walkingRoutePolyline: any = null;
	private walkingRouteFitFrame = 0;
	...
	clearWalkingRoute() {
		if (this.walkingRouteFitFrame) {
			cancelAnimationFrame(this.walkingRouteFitFrame);
			this.walkingRouteFitFrame = 0;
		}
		if (this.walkingRoutePolyline) {
			this.walkingRoutePolyline.setMap?.(null);
			this.walkingRoutePolyline = null;
		}
	}

	drawWalkingRoute(points: Array<{ latitude: number; longitude: number }>) {
		this.clearWalkingRoute();
		if (!this.map || typeof Tmapv3 === 'undefined' || points.length < 2) return;

		const path = points.map((point) => new Tmapv3.LatLng(point.latitude, point.longitude));
		this.walkingRoutePolyline = new Tmapv3.Polyline({
			path,
			strokeColor: '#1965e9',
			strokeWeight: 8,
			map: this.map,
		...
		this.walkingRouteFitFrame = requestAnimationFrame(() => {
			this.walkingRouteFitFrame = requestAnimationFrame(() => {
				this.walkingRouteFitFrame = 0;
				this.fitWalkingRouteInVisibleArea(points);
			});
		});
	}

	private fitWalkingRouteInVisibleArea(points: Array<{ latitude: number; longitude: number }>) {
		if (!this.map || typeof Tmapv3 === 'undefined') return;
		...
		const padded = expandBoundsForOverlayPadding(rawBounds, mapSize, measureWalkingRouteOverlayPadding(container));

		this.map.fitBounds?.(
			new Tmapv3.LatLngBounds(
				new Tmapv3.LatLng(padded.minLat, padded.minLng),
				new Tmapv3.LatLng(padded.maxLat, padded.maxLng),
			),
		);
	}
	...
	clearFacilityPins() {
		this.facilityPins.forEach((marker) => {
			marker?.setMap?.(null);
			marker?.setVisible?.(false);
		});
		this.facilityPins.clear();
	}
	...
	replaceFacilityPins(
		items: Array<{
			...
			longitude: number;
			poiName: unknown;
			category?: { iconKey?: string | null; categoryColorCodes?: { colorCode?: string | null } | null } | null;
			congestionStatus?: string;
		}>,
		onClick?: (item: { poiId: number; facilityId: number; latitude: number; longitude: number; poiName: unknown }) => void,
		selected?: { poiId?: number; facilityId?: number },
	) {
		this.clearFacilityPins();

		items.forEach((item) => {
			const label =
				item.poiName && typeof item.poiName === 'object' && 'ko' in item.poiName
					? String((item.poiName as { ko?: string }).ko ?? '')
					: '';
			const isSelected =
				(Number(selected?.facilityId) > 0 && item.facilityId === selected?.facilityId) ||
				(Number(selected?.poiId) > 0 && item.poiId === selected?.poiId);

			const marker = this.makerPin(
				item.latitude,
				item.longitude,
				item.category?.iconKey ?? 'location',
				item.congestionStatus ?? 'none',
			...
			this.facilityPins.set(item.facilityId, marker);
			marker.on?.('Click', () => {
				onClick?.(item);
			});
		});
