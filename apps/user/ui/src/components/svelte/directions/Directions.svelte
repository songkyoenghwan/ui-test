<script lang="ts">
	...
	import { closeWalkingRoute, openWalkingPlaceSearch, swapWalkingEndpoints } from '@/stores/pageDataStore';
	import { searchViewState, toastState } from '@/stores/uxStore';
	import Icons from '@/svelte/icons/Icons.svelte';

	function swapDistance() {
		swapWalkingEndpoints();
	}
				...
				>
					...
					>
						...
						<input
							...
							onclick={() => {
								if ($searchViewState === 'departure') openWalkingPlaceSearch('start');
							}}
						/>
					</li>
					...
					>
						...
						<input
							...
							onclick={() => {
								if ($searchViewState === 'departure') openWalkingPlaceSearch('end');
							}}
						/>
					</li>
				</ol>
...
{#if ['same', 'disabled', 'far', 'arrival'].includes($toastState)}
	<div
		class={[
			...
			($searchViewState === 'path' || $searchViewState === 'departure') && 'top-23',
		]}
	>
		<div
			class={[
