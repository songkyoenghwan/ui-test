<script lang="ts">
	...
	import { destinationDetail } from '@/stores/pageDataStore';
	...
	import DistanceGuide from '@/svelte/sheet/DistanceGuide.svelte';
	import Facility from '@/svelte/sheet/Facility.svelte';
	import SearchSheet from '@/svelte/sheet/SearchSheet.svelte';
	import DistanceFinding from '@/svelte/sheet/DistanceFinding.svelte';
	...
	const bottomValueNow: Attachment = (element) => {
		...
		sheetInstance.set(currentSheet);
		...
		return () => {
			observer.disconnect();
			if (sheetInstance.get() === currentSheet) {
				sheetInstance.set(undefined);
			}
		};
	};
...
{#if ['default', 'ai', 'facilities', 'search', 'directions'].includes($detailViewState)}
	<div
	...
	>
		<BottomSheet bind:this={sheet} settings={sheetSettings} bind:isSheetOpen={$sheetUi.sheetHandleOpen} onclose={keepOpen}>
			<BottomSheet.Sheet>
				...
				{#if ['idle', 'default', 'ai', 'search', 'path', 'directions'].includes($detailViewState)}
					<ControlGroup
