<script lang="ts">
	...
	import type { BasicSurveyAnswerRequest, TourDestinationBasicSurveyQuestionResponse } from '@/types/tour-destinations';

	let data = $derived($tourDestinationBasicSurvey);
	let submissionState = $derived($basicSurveySubmissionState);
	let current = $state(0);
	let prevCurrent = $state(0);
	let currentStep = $state<'plus' | 'minus'>('plus');
	let answers = $state<Record<number, string | string[]>>({});
	let surveyList = $derived(data?.questions ?? []);
	let listTotal = $derived(current === surveyList.length - 1 ? false : true);
	let listStart = $derived(current > 0);
	let isSubmitting = $derived(submissionState !== 'idle');
	const linkUrl = '/map';

	const hasValidAnswer = (survey: TourDestinationBasicSurveyQuestionResponse | undefined) => {
		if (!survey) return false;

		const value = answers[survey.id];
		if (survey.questionType === 'MULTI') {
	...
	let isCurrentAnswered = $derived.by(() => {
		return hasValidAnswer(surveyList[current]);
	});
	let areAllQuestionsAnswered = $derived(surveyList.length > 0 && surveyList.every((survey) => hasValidAnswer(survey)));
	...
	$effect(() => {
		...
		resetSurveyState();
		void loadTourDestinationBasicSurvey(destinationId, language, controller.signal);
	...
	const createAnswerPayload = (): BasicSurveyAnswerRequest[] | null => {
		const answerPayload: BasicSurveyAnswerRequest[] = [];
		...
		for (const survey of surveyList) {
			...
			if (optionIds.length === 0 || (survey.questionType === 'SINGLE' && optionIds.length !== 1)) {
				return null;
			}

			answerPayload.push({ questionId: survey.id, optionIds });
		}

		return answerPayload.sort((left, right) => left.questionId - right.questionId);
	};
	...
	const handleSubmit = async (event: MouseEvent) => {
		...
		if (!areAllQuestionsAnswered || !answerPayload) return;

		const submitted = await submitTourDestinationBasicSurvey(getCurrentTourDestinationId(), $langState, answerPayload);
		if (submitted) window.location.assign('/map');
	};

	const handleSkip = async (event: MouseEvent) => {
		event.preventDefault();
		if (isSubmitting) return;

		const skipped = await skipTourDestinationBasicSurvey(getCurrentTourDestinationId(), $langState);
		if (skipped) window.location.assign('/map');
	};
</script>
...
<div class="group/lang flex min-h-0 flex-1 flex-col" data-lang={$langState}>
	...
	<footer class="flex-none">
		...
		>
			...
			{:else}
				{#if areAllQuestionsAnswered}
					<a
						href={linkUrl}
						class="h-10 flex-1 justify-center gap-2.5 rounded-sm bg-(--base-color) text-white"
		...
		<div class="text-center">
			<a href={linkUrl} class="flex justify-center py-1 text-sm text-slate-500" onclick={handleSkip}>
