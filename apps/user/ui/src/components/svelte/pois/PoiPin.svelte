	...
	<span class="-mt-6 flex flex-col items-center justify-center gap-1" style:--category-color={color}>
		...
		<span class="relative">
			<svg
				...
				fill="none"
				xmlns="http://www.w3.org/2000/svg"
				class="fill-(--category-color) stroke-(--category-color)"
			>
				<g opacity="0.6">
			...
			<span class="absolute top-3.25 left-3.5 z-1">
				<IconCategory reverse="poi-current" icon={category} {color} />
			</span>
		</span>
...
{#snippet categoryPin()}
	<span class="relative flex flex-col items-center justify-center gap-1" style:--category-color={color}>
		...
		<span class="line-clamp-2 max-w-26.25 text-xs/tight leading-3.25 [word-break:auto-phrase] text-black text-shadow-white">
			{label}
		</span>
	</span>
{/snippet}
