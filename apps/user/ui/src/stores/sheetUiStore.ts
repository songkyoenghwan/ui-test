...
import { aiRecommendationFacilityItems, pathList, searchResultList } from '@/stores/pageDataStore';
...
export const sheetMidRatioValue = computed([sheetUi, viewportH], (state, vh) => {
	...
	if (detailViewState.get() === 'ai') {
		if (aiRecommendationFacilityItems.get().length > 0) {
