...
/** 대상지 미선택 시 지도 진입 기본값. 해당 대상지가 없으면 FALLBACK을 쓴다. */
export const DEFAULT_TOUR_DESTINATION_ID = 48;
export const FALLBACK_TOUR_DESTINATION_ID = 1;

export const currentTourDestinationId = persistentAtom<number>('currentTourDestinationId', DEFAULT_TOUR_DESTINATION_ID, {
...
export function resolveMapDestinationId(): number {
	const destinationId = Number(currentTourDestinationId.get());
	return Number.isInteger(destinationId) && destinationId > 0 ? destinationId : DEFAULT_TOUR_DESTINATION_ID;
}
