<script lang="ts">
	...
	import { aiInterestMatching, aiInterest } from '@/stores/uxStore';

	import Icons from '@/svelte/icons/Icons.svelte';
	import { recommendList, popularityList } from '@/stores/pageDataStore';
...
<div class="p-5" bind:clientHeight={null, setSheetMidH}>
	{#if hasSetInitialSnapPoint && $recommendList.length === 0}
...
{#if tab === 'ai-recommend'}
	<ul class="divide-y-2 divide-slate-200 border-t-2 border-t-slate-200 empty:hidden">
		{#each $recommendList.slice(0, 5) as result (result.id)}
			<!-- svelte-ignore a11y_no_noninteractive_element_to_interactive_role -->
			<li class="py-2 first:pt-0" role="button" aria-label="Button">
...
{:else if tab === 'popularity'}
	<ul class="divide-y-2 divide-slate-200 border-t-2 border-t-slate-200 empty:hidden">
		{#each $popularityList.slice(0, 5) as result (result.id)}
			<!-- svelte-ignore a11y_no_noninteractive_element_to_interactive_role -->
			<li class="py-2 first:pt-0" role="button" aria-label="Button">
				<Info variant="list" rest={result} />
			</li>
