...
{#if $facilityCurrent?.facility?.description}
		...
		<IntersectionObserver {element} bind:intersecting>
			<div class="relative rounded-lg bg-slate-100 p-3 transition-all">
				<div
					bind:this={element}
					class={[
						'relative overflow-clip bg-slate-100 transition-all',
						open ? 'max-h-max' : 'max-h-17.5 overflow-clip',
					]}
				>
					<p
					...
					>
						{pickText($facilityCurrent?.facility?.description, $langState)}
					</p>
				</div>
