<script lang="ts">
	...
	import type { FacilityDetailResponse, FacilityHolidayScheduleResponse } from '@/types/facilities';
	...
	//	let currentData = $derived(($facilityList ?? []).find((f) => f.id === data?.id));
	let variantData = $derived(variant === 'list' ? data : $facilityCurrent?.facilityDetail);
	...
	const operationTime = $derived.by(() => {
		return variantData?.facilityOperatingSchedules
	...
	const isAlwaysOperating = $derived(
		ALL_DAYS.every((dayOfWeek) =>
			variantData?.facilityOperatingSchedules?.some(
				(schedule) =>
	...
	// 시설의 운영 기간 시작일과 종료일
	const startAt = $derived(variantData?.startAt);
	const endAt = $derived(variantData?.endAt);
	...
	// 현재 요일에 등록된 운영 스케줄
	const todaySchedule = $derived(variantData?.facilityOperatingSchedules?.find((schedule) => schedule.dayOfWeek === now.day()));
	// 오늘 운영 스케줄의 시작·종료 시각
	...
	const isHoliday = $derived(variantData?.facilityHolidaySchedules?.some((holiday) => isCurrentHoliday(now, holiday)) ?? false);
	const holidaySummaries = $derived.by(() => {
		const holidays = variantData?.facilityHolidaySchedules ?? [];
		...
		for (const holiday of fixedHolidays.sort((a, b) => (a.fixedHoliday ?? 0) - (b.fixedHoliday ?? 0))) {
			summaries.push(
				`${m.usr_map_002_37({ time: pickText(holiday?.fixedHoliday, $langState) }, { locale: $langState })} ${m.usr_map_002_38({ locale: $langState })}`,
			);
		}
...
{#snippet currentState()}
	...
	{:else if isBreakTime}
		...
		{#if currentBreak?.breakEndTime}
			<span class="text-sm font-normal text-slate-500">
				<span aria-hidden="true">&middot</span>
				{m.usr_map_002_14({ time: normalizeClosingTime(currentBreak.breakEndTime) }, { locale: $langState })}
			</span>
		{/if}
	{:else if isBeforeOpen}
		{m.usr_map_002_15({ locale: $langState })}
	...
	{:else}
		...
		{#if todaySchedule?.closingTime}
			<span class="text-sm font-normal text-slate-500">
				<span aria-hidden="true">&middot</span>
				{m.usr_map_002_12({ time: normalizeClosingTime(todaySchedule.closingTime) }, { locale: $langState })}
			</span>
		{/if}
	{/if}
{/snippet}
...
{#if variant === 'list'}
	<span class="flex items-center gap-1 px-5 py-1 text-sm font-bold text-black">
		<Icons name="clock-filled" cls="size-4 fill-slate-400 stroke-slate-400" />
		{@render currentState()}
	</span>
{:else}
	{#if (variantData?.facilityOperatingSchedules?.length ?? 0) > 0}
		{#if variant === 'detail'}
			<span class="flex items-center gap-1 px-5 py-1 text-sm font-bold text-black">
				<Icons name="clock-filled" cls="size-4 fill-slate-400 stroke-slate-400" />
				{@render currentState()}
		...
		{:else}
			<dl class="flex flex-col gap-3 px-5 py-1.5">
				...
				>
					{#if isAlwaysOperating || (variantData?.facilityOperatingSchedules?.length ?? 0) === 0}
