		...
		<section class="map-list">
			...
			<div class="flex items-center justify-between gap-1 px-5 py-3">
				...
				<p class="flex items-center gap-1">
					...
					<ui-txt size="sm" cls="text-cms-3 font-bold" txt={String(data.totalCount)}></ui-txt>
				</p>
			</div>
						...
						{#each data.items as item, index (item?.id)}
							<li data-index={index} class="w-full border-b border-b-slate-200">
								<button
									...
									onclick={() => {
										facilitiesCurrent = data.items.find((v) => v.id === item.id);
									}}
								>
									...
									>
										...
										<span class="flex items-center gap-1 text-slate-500">
											...
											<strong class="flex items-center">
												<span
													class={(item.facilityPoiMappings.length ?? 0) === 0
														? 'text-error'
														: 'text-cms-3'}
												>
													(item.facilityPoiMappings.length ?? 0)
												</span>
	...
	<div class="map-reg">
		...
		<section class="group/map-lang flex flex-col gap-2 border-t border-t-slate-200 px-5 py-3">
			<div class="flex items-center justify-between gap-1">
				...
				<ui-txt size="sm" txt={`POI ${facilitiesCurrent.id ?? '이름없음'}`}></ui-txt>
			</div>
			<div class="flex flex-col gap-4">
				<lang-translate data-max-length="25" view="reg" lang={facilitiesCurrent.name}></lang-translate>
				...
				<ul class="flex flex-col gap-1">
					<li class="grid grid-cols-[22px_1fr_22px_1fr] items-center gap-2">
						...
						<input
							...
							class="input-text s"
							placeholder="위도"
							bind:value={facilitiesCurrent.lat}
							maxlength="9"
						/>
						...
						<input
							...
							class="input-text s"
							placeholder="경도"
							bind:value={facilitiesCurrent.lon}
							maxlength="9"
						/>
					</li>
					<li class="grid grid-cols-[22px_1fr] items-center gap-2">
						...
						<input
							...
							placeholder="주소"
							readonly
							bind:value={facilitiesCurrent.addr}
						/>
					</li>
				</ul>
			</div>
		...
		<section class="flex min-h-0 flex-1 flex-col gap-4 border-t border-t-slate-200 py-4">
			<header class="grid grid-cols-2 items-center gap-2.5 px-5">
				...
				<p class="flex-1 text-right text-slate-500">
					<strong class="text-cms-5 font-bold">{facilitiesCurrent.facilityPoiMappings.length}</strong>
					<span>개</span>
				</p>
			...
			<ul class="flex flex-1 flex-col gap-2 overflow-x-clip overflow-y-auto scroll-smooth">
				{#each facilitiesCurrent.facilityPoiMappings as mapping}
					<li
					...
					>
						...
						<div class="grid w-full space-y-0.5 pl-1">
							<p class="truncate text-sm text-slate-600">{mapping.poi.name.ko ?? ''}</p>
