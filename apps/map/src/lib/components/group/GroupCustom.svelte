...
<ul class="divide-y divide-slate-300">
		...
		<div class="grid max-w-375 grid-cols-[100px_1fr] gap-5 px-4 py-5">
			...
			<ul class="inline-grid divide-y divide-slate-200">
				...
				<li class="flex items-center justify-between gap-2 p-3">
					<Chk itemId="zone-congestion" txt="구역 혼잡도" reverse="true" cls="min-w-32.5 min-h-9" bind:checked={result.zoneCongestion} />
					<ui-txt size="sm" txt="구역에 대한 혼잡도 정보를 안내하며, 지도 관리 메뉴에서 구역을 설정 후 실제 혼잡도 데이터를 불러올 수 있습니다"></ui-txt>
				</li>
				<li class="flex items-center justify-between gap-2 p-3">
					<Chk itemId="facility-congestion" txt="시설 혼잡도" reverse="true" cls="min-w-32.5 min-h-9" bind:checked={result.facilityCongestion} />
	...
	<li>
		<div class="grid max-w-375 grid-cols-[100px_1fr] gap-5 px-4 py-4">
			...
			<ul class="inline-grid divide-y divide-slate-200">
				<li class="flex items-center justify-between gap-2 px-3 pb-3">
					<Chk itemId="location-based-content" txt="위치 기반 콘텐츠" reverse="true" cls="min-w-32.5 min-h-9" bind:checked={result.locationBased} />
					<ui-txt size="sm" txt="AI 기반으로 사용자 맞춤 시설을 추천하며, 카테고리별 추천 노출 여부를 설정할 수 있습니다"></ui-txt>
				</li>
				<li class="flex items-center justify-between gap-2 p-3">
					<Chk itemId="facility-address-exposure" txt="시설 주소 노출" reverse="true" cls="min-w-32.5 min-h-9" bind:checked={result.facilityAddress} />
					<ui-txt size="sm" txt="구역에 대한 혼잡도 정보를 안내하며, 지도 관리 메뉴에서 구역을 설정 후 실제 혼잡도 데이터를 불러올 수 있습니다"></ui-txt>
				</li>
				<li class="flex items-center justify-between gap-2 p-3">
	...
	<li>
		<div class="grid max-w-375 grid-cols-[100px_1fr_120px] items-center gap-5 px-4 py-4">
			...
			<ui-txt size="sm" txt="시설 등록 시 설정된 버튼에 대한 링크를 삽입하여 사용자에게 제공할 수 있으며, 최대 3개까지 등록 가능합니다"></ui-txt>
			<UiBtn variant="secondary" size="lg" txt="추가" cls="min-w-30" click={handleAddBtnLink} disabled={result.btnLink?.length === 3} />
		</div>
