...
function YR(e, t) {
	...
	return G(g, (e) => {
		...
		K(u, 1, e), K(d, 1, ea(o() ? o() : "space-y-2.5")), W(p, V(s)), W(h, i() ? i() : V(c));
	}, [() => ea([
...
function Az(e, t) {
	...
	N(_), N(m);
	var y = R(m, 2), b = L(y);
	b.muted = !0, q(L(b), "src", "/output/video/ceo-video.mp4"), N(b), N(y);
	var x = R(y, 2), S = L(x);
	...
	var T = R(w, 2), E = L(T), ee = L(E), D = L(ee, !0);
	N(ee);
	var O = R(ee, 2), te = L(O), k = L(te), A = L(k, !0);
	N(k);
	var ne = R(k, 2);
...
function JU(e, t) {
	...
	var s = R(o, 2);
	J(s, "slides-per-view", 1), J(s, "space-between", 10), J(s, "centered-slides", !0), J(s, "speed", "450"), J(s, "pagination", { hideOnClick: !0 }), J(s, "data-scroll", "slide-up");
	var c = L(s), l = (e) => {
		var t = ci();
	...
	Ai(g, 23, () => V(n).slice(0, 4), (e, t) => `dot-${t}`, (e, t, n) => {
		...
		N(o), N(r), z(() => {
			i = K(r, 1, "size-2 rounded-full shadow-md transition-all duration-300", null, i, {
				"bg-white": (V(a)?.swiper?.realIndex ?? 0) % 4 === V(n),
				"bg-9097ff": (V(a)?.swiper?.realIndex ?? 0) % 4 !== V(n),
				"w-6": V(s) === V(n)
			}), W(c, `${V(n) + 1}번째 슬라이드 구역`);
		}), U(e, r);
	}), N(g);
	var _ = R(g, 2), v = (e) => {
		...
		J(o, "name", "arrow-right"), K(o, 1, "size-6 stroke-white"), N(i), N(t), Zr("click", n, () => {
			V(a)?.swiper?.slidePrev();
		}), Zr("click", i, () => V(a)?.swiper?.slideNext()), U(e, t);
	};
	G(_, (e) => {
		V(o) && e(v);
	}), N(m), N(c), z((e, t) => {
...
function VK(e, t) {
	...
	])), r = /* @__PURE__ */ I(xn([])), i = /* @__PURE__ */ new Map(), a = async (e) => {
		let t = V(r)[e];
	...
	Ai(c, 23, () => V(n), (e) => e.id, (e, t, n) => {
		...
		var _ = R(h, 2), v = (e) => {
			var n = RK(), r = L(n), i = L(r), a = (e) => {
				...
				var i = R(r, 2);
				N(n), z(() => {
					q(i, "src", V(t).img.img.src), K(i, 1, ea(["relative ", V(t).id === "why-3" ? "relative h-full translate-x-4/10 translate-y-[15%] scale-180 transition-all max-lg:w-full lg:translate-x-[40%] lg:-translate-y-1/5 lg:scale-130 lg:group-hover/why:-translate-y-[calc(20%+30px)]" : V(t).id === "why-4" ? "h-full flex-1 max-lg:max-w-59 lg:min-h-61.5" : ""])), q(i, "width", V(t).img.img.w), q(i, "height", V(t).img.img.h);
				}), U(e, n);
			};
			G(i, (e) => {
				typeof V(t).img == "string" ? e(a) : e(o, -1);
			}), N(r), N(n), z(() => K(r, 1, ea(["relative flex flex-1 justify-end", V(t).id === "why-4" ? "flex h-full max-h-43.5 flex-wrap items-end justify-center px-5 lg:w-124.75 lg:translate-y-15 lg:p-5" : ""]))), U(e, n);
		};
		G(_, (e) => {
			V(t).img && e(v);
		}), N(i), z(() => {
			q(i, "data-index", V(n)), K(i, 1, ea(["group/why relative flex h-90 w-full justify-between overflow-clip rounded-xl transition-all duration-300 max-lg:flex-col lg:h-full lg:hover:scale-105", V(t).cls])), K(s, 1, ea(["flex items-center text-5xl leading-none font-bold lg:text-6xl lg:data-[font=90]:text-[90px]", V(t).id === "why-3" ? "text-primary" : ""])), q(s, "data-font", V(t).font), W(f, V(t).id === "why-2" || V(t).id === "why-3" ? "+" : ""), K(p, 1, ea([V(t).id !== "why-3" && V(t).id !== "why-4" ? "text-2md lg:text-lg" : "text-lg lg:text-2xl"])), W(m, V(t).txt);
