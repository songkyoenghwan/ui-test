...
const rotateSlides = (emblaApi: EmblaCarouselType, loop: boolean, slideCount: number, totalRadius: number): (() => void) => {
	return (): void => {
...
const rotateWheel = (emblaApi: EmblaCarouselType, slideCount: number, rotationOffset: number): (() => void) => {
	return (): void => {
...
export const setupIosPicker = (iosPickerNode: HTMLElement, options: EmblaOptionsType): EmblaCarouselType => {
