...
<script lang="ts">
			...
			instance = flatpickr(input, {
				...
				onChange: (selectedDates, dateStr, inst) => {
					if (config.dateType === 'range' && selectedDates.length === 2) {
						...
						day = `${start}~${end}`;
						startAt = String(selectedDates[0].toISOString());
						endAt = String(selectedDates[1].toISOString());
					}

					if (config.dateType === 'default' && selectedDates.length === 1) {
						day = dateStr;
						startAt = String(selectedDates[0].toISOString());
					}
				},
	...
	$effect(() => {
		...
		const single = toValidDate(startAt);

		if (isValidDate(single)) {
			instance.setDate(single, false);
		} else {
			instance.clear();
		}
	});
